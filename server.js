...
app.post('/api/subscribe', async (req, res) => {
  ...
  try {
    ...
    if (RESEND_AUDIENCE_ID) {
      ...
      if (!audienceRes.ok) {
        console.error('Audience add error:', await audienceRes.text());
      }
    }

    // 2. Send Campaign 3 — Email 1
...
// SPA fallback
app.get('*', (req, res) => {
  const htmlFile = req.path === '/thank-you' ? 'thank-you.html' : 'index.html';
  res.sendFile(path.join(__dirname, htmlFile));
});

createServer(app).listen(PORT, () => {
  console.log(`Bequall prompt pack landing running on port ${PORT}`);
});
...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="prompts">
      ...
      <ul>
        <li>First-pass deal screen</li><li>IC memo first draft</li><li>Comparable analysis narrative</li>
        <li>LP update draft</li><li>Investor FAQ response</li><li>Monthly reporting narrative</li>
        <li>Meeting summary + action items</li><li>Scope gap analysis</li>
        <li>Site visit notes → report</li><li>Email response drafts</li>
        <li>Executive briefing summary</li><li>SOP / process documentation</li>
      </ul>
    </div>
  ...
  <div class="footer">
    <p>You're receiving this because you downloaded the AI Operating Baseline Prompt Pack at bequall.com. One optional follow-up. <a href="#">Unsubscribe</a>.</p>
  </div>
</div>
</body>
</html>`;
